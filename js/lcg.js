...
export default class LCG {
	...
	constructor(
		...
		if (seed == undefined) {
			throw new Error('please enter an integer seed for LCG');
		}
		...
		if (typeof this.X0 == 'string') {
			...
			if (Number.isNaN(this.X0)) {
				throw new Error(`Cannot convert seed (${this.X0}) to an integer.`);
			}
		...
		if (!Number.isInteger(this.X0)) {
			throw new Error(`Not able to convert seed (${this.X0}) to an integer`);
		} else {
			console.log(
